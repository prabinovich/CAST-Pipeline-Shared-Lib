	...
	node ('master') {
	    ...
	    stage ('CAST-Assessment Model') {
	    	try {
	        	...
	        	bat '"%WORKSPACE%\\CAST-Scripts\\CLI-Scripts\\CMS_ImportAssessmentModel.bat" "profile=${config.cmsprofile}" "app=${config.appname}" "filepath=%WORKSPACE%\\CAST-Scripts\\QualityModels\\CAST 8.3.8 Assessment Model - Standard.pmx"'
	    	}
	    	catch (err) {
	    ...
	    stage('CAST-Publish Results'){
	        ...
	        withCredentials([usernamePassword(credentialsId: 'CAST-DB-Keys', passwordVariable: 'PWD1', usernameVariable: 'USR1')]) {
	            bat '"%WORKSPACE%\\CAST-Scripts\\CLI-Scripts\\AAD_ConsolidateSnapshot.bat" "measure=${config.mesure}" "central=${config.central}" "password=%PWD1%"'
	        }
